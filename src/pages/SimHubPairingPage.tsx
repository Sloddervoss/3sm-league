...
const SimHubPairingPage = () => {
  ...
  const staff = isAdmin || isSuperAdmin || isSteward;
  ...
  return <div className="flex min-h-screen flex-col bg-background text-foreground">
    ...
    <main className="container mx-auto flex-1 px-4 pb-16 pt-28">
      <div className="mx-auto max-w-4xl">
        ...
        {(loading || rolesLoading) ? <div className="rounded-2xl border border-border bg-card p-8 text-center"><Loader2 className="mx-auto h-7 w-7 animate-spin text-orange-400" /><p className="mt-3 text-sm text-muted-foreground">Account laden…</p></div> : !user ? <div className="rounded-2xl border border-orange-500/20 bg-orange-500/[0.07] p-8 text-center"><Cable className="mx-auto h-10 w-10 text-orange-400" /><h2 className="mt-4 text-xl font-bold text-white">Log eerst in</h2><p className="mt-2 text-sm text-gray-400">Pairingcodes zijn kort geldig en worden tijdens de beta door 3SM-staff beheerd.</p><Link to="/auth?redirect=/simhub-koppelen" className="mt-6 inline-flex rounded-lg bg-orange-500 px-5 py-2.5 text-sm font-bold text-black hover:bg-orange-400">Inloggen</Link></div> : !staff ? <div className="rounded-2xl border border-amber-500/20 bg-amber-500/[0.07] p-8 text-center"><ShieldCheck className="mx-auto h-10 w-10 text-amber-300" /><h2 className="mt-4 text-xl font-bold text-white">Staffcode vereist</h2><p className="mt-2 text-sm text-gray-400">Tijdens de gecontroleerde beta maakt een admin, super-admin of moderator de code voor iedere tester. Communityteamlidmaatschap geeft bewust geen telemetrytoegang.</p></div> : <div className="space-y-5">
