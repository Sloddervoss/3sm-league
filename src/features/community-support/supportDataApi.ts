...
type RaceRow = {
  ...
  exchange_rate_usd_eur: number; vat_rate: number; vat_amount_usd: number; gross_amount_usd: number; net_amount_eur: number; vat_amount_eur: number; amount_eur: number; is_public: boolean; note: string | null;
...
const raceFromRow = (row: RaceRow): SupportRaceCost => ({
  ...
  exchangeRateUsdEur: Number(row.exchange_rate_usd_eur), vatRate: Number(row.vat_rate), vatAmountUsd: Number(row.vat_amount_usd),
  grossAmountUsd: Number(row.gross_amount_usd), netAmount: Number(row.net_amount_eur),
