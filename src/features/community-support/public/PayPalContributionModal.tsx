...
const copyFor = (language: Language) => language === "en" ? {
  ...
  title: "Contribute through PayPal.Me",
  intro: "Choose an amount and your public preferences first. PayPal opens securely in a separate tab or app; this 3SM window stays ready for your return.",
  amount: "Amount",
  ...
  openPayPal: "Continue securely to PayPal",
  invalid: "Enter an amount of at least €1 and the name the admin will see in PayPal.",
  ...
  doneLive: "The payment admin will receive a private Discord message and will add the contribution only after checking PayPal.",
  ...
  title: "Bijdragen via PayPal.Me",
  intro: "Kies eerst een bedrag en je openbare voorkeuren. PayPal opent veilig in een apart tabblad of de app; dit 3SM-venster blijft klaarstaan voor je terugkomst.",
  amount: "Bedrag",
  ...
  openPayPal: "Ga veilig verder naar PayPal",
  invalid: "Vul minimaal €1 in en de naam die de admin in PayPal zal zien.",
  ...
  doneLive: "De betaaladmin ontvangt privé een Discord-bericht en voegt de bijdrage pas toe na controle in PayPal.",
...
const PayPalContributionModal = ({ language, settings, localReview, canOpenPayPal, onAuthenticationRequired, onSubmit }: Props) => {
  const copy = copyFor(language);
  const [stage, setStage] = useState<"choose" | "paypal" | "done">("choose");
  ...
  const valid = Boolean(paypalUrl && payerName.trim());
  ...
  return <div className="min-h-[560px] p-6 sm:p-8">
    <div className="pr-10">
      ...
      <h2 ref={stageHeadingRef} tabIndex={-1} className="mt-3 font-heading text-2xl font-black uppercase text-white outline-none sm:text-3xl">{stage === "choose" ? copy.title : stage === "paypal" ? copy.waitingTitle : copy.doneTitle}</h2>
    </div>

    <div aria-live="polite" aria-atomic="false" aria-label={copy.progressLabel}>
    {stage === "choose" && <>
      <p className="mt-4 max-w-2xl text-sm leading-6 text-gray-400">{copy.intro}</p>
      ...
      <label className="mt-5 block text-xs font-bold text-gray-400">{copy.payerName}<input value={payerName} maxLength={100} autoComplete="name" onChange={(event) => setPayerName(event.target.value)} className={inputClass} /><span className="mt-2 block font-normal leading-5 text-gray-500">{copy.payerHelp}</span></label>
      ...
      <button type="button" onClick={openPayPal} className="mt-7 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-gradient-racing px-5 text-sm font-black text-white shadow-lg shadow-orange-950/30 transition hover:brightness-110 focus-visible:ring-2 focus-visible:ring-orange-300 sm:w-auto"><ExternalLink className="h-4 w-4" aria-hidden="true" />{copy.openPayPal}</button>
    </>}
    ...
    {stage === "done" && <div role="status" className="mt-8 rounded-2xl bg-emerald-500/[0.07] p-6 ring-1 ring-emerald-400/20">
      ...
      <p className="mt-4 text-sm leading-6 text-gray-300">{localReview ? copy.doneLocal : copy.doneLive}</p>
    </div>}
    </div>

    <div className="mt-7 flex items-start gap-3 rounded-xl bg-sky-400/[0.04] p-4 text-xs leading-5 text-gray-400 ring-1 ring-sky-300/10"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-sky-300" aria-hidden="true" />{copy.integrity}</div>
  </div>;
};
