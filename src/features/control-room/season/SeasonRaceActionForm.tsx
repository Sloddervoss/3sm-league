...
type RaceRow = {
  id: string; league_id: string | null; name: string; track: string; iracing_track_id: number | null; race_date: string | null; round: number | null; status: string | null;
...
type RaceForm = {
  name: string; track: string; iracing_track_id: number | null; race_date: string; race_type: string; race_duration: string; practice_duration: string;
...
const defaultSlot = (name = "", defaults: SeasonRaceDefaults = DEFAULT_SEASON_RACE_SETTINGS): RaceSlot => ({
  name, track: "", iracing_track_id: null, date: "", time: "20:00", ...defaults, lobby_name: "", lobby_password: "",
});

const defaultRaceForm = (): RaceForm => ({ ...defaultSlot(), race_date: "", status: "upcoming" });

const raceToForm = (race: RaceRow): RaceForm => ({
  name: race.name || "", track: race.track || "", iracing_track_id: race.iracing_track_id, race_date: race.race_date ? utcToAmsLocal(race.race_date) : "",
...
const standardPayload = (form: RaceForm) => ({
  name: form.name,
  track: form.track,
  iracing_track_id: form.iracing_track_id,
...
const slotPayload = (slot: RaceSlot, leagueId: string | null, round?: number) => ({
  ...
  name: slot.name,
  track: slot.track,
  iracing_track_id: slot.iracing_track_id,
...
export const SeasonRaceActionForm = ({ action, onComplete }: SeasonRaceActionFormProps) => {
  ...
  const { data: races = [] } = useQuery({
    ...
    queryFn: async (): Promise<RaceRow[]> => {
      const { data, error } = await supabase.from("races").select("id,league_id,name,track,iracing_track_id,race_date,round,status,race_type,race_duration,practice_duration,qualifying_duration,start_type,weather,setup,lobby_name,lobby_password,lobby_reveal_minutes");
      if (error) throw error;
  ...
  return <section aria-label="Gerichte seizoen- of raceactie" className="mx-auto max-w-4xl space-y-6 text-gray-100">
    ...
    <button type="button" onClick={submit} disabled={pending || (isSeasonCreate && (!leagueForm.name || slots.some((slot) => !slot.iracing_track_id))) || ((isRaceCreate || isSoloCreate) && (!raceForm.name || !raceForm.iracing_track_id || !raceForm.race_date))} className="flex items-center gap-2 rounded-lg bg-gradient-racing px-5 py-3 text-sm font-black text-white shadow-lg shadow-orange-950/30 disabled:cursor-not-allowed disabled:opacity-50"><Save className="h-4 w-4" />{pending ? "Opslaan…" : isSeasonCreate || isRaceCreate || isSoloCreate ? "Aanmaken" : "Opslaan"}</button>
  </section>;
};
