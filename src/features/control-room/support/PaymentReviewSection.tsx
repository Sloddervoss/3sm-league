...
const PaymentReviewSection = ({ language, settings, intents, localReview, isSuperAdmin, onUpdateSettings, onResolve }: Props) => {
  ...
  const [draftCheckoutEnabled, setDraftCheckoutEnabled] = useState(settings.paypalCheckoutEnabled);
  const [draftCheckoutEnvironment, setDraftCheckoutEnvironment] = useState(settings.paypalCheckoutEnvironment);
  ...
  useEffect(() => {
    ...
    setDraftCheckoutEnabled(settings.paypalCheckoutEnabled);
    setDraftCheckoutEnvironment(settings.paypalCheckoutEnvironment);
  ...
  const save = async (event: FormEvent) => {
    ...
    try {
      await onUpdateSettings({
        ...
        paypalCheckoutEnabled: draftCheckoutEnabled,
        paypalCheckoutEnvironment: draftCheckoutEnvironment,
  ...
  return <section id="panel-payments" role="tabpanel" className="space-y-6">
    <div><div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-orange-400"><CircleDollarSign className="h-4 w-4" />Community Support</div><h2 className="mt-2 font-heading text-2xl font-black text-white">{language === "en" ? "Payments and referral" : "Betalingen en referral"}</h2><p className="mt-2 max-w-3xl text-sm leading-relaxed text-gray-400">{draftCheckoutEnabled ? (language === "en" ? "Configure direct server-verified PayPal Checkout and the separate iRacing referral link." : "Configureer directe, server-side geverifieerde PayPal Checkout en de losse iRacing-referrallink.") : (language === "en" ? "Configure the public PayPal.Me flow, payment checks and the separate iRacing referral link." : "Configureer de openbare PayPal.Me-flow, betaalcontroles en de losse iRacing-referrallink.")}</p></div>

    <form onSubmit={(event) => void save(event)} className={`${card} space-y-6 p-6 md:p-8`}>
      <div className="flex items-start gap-4"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-orange-500/10 text-orange-300"><MessageCircle className="h-5 w-5" /></div><div><h3 className="font-heading text-lg font-black">{draftCheckoutEnabled ? (language === "en" ? "Checkout configuration" : "Checkoutconfiguratie") : (language === "en" ? "Payment destination and private admin" : "Betaalbestemming en privé-admin")}</h3><p className="mt-1 text-sm leading-6 text-gray-500">{draftCheckoutEnabled ? (language === "en" ? "Checkout captures and verifies payments automatically on the server. The PayPal.Me link and Discord ID below are retained only for the independent manual flow." : "Checkout verwerkt en verifieert betalingen automatisch op de server. De PayPal.Me-link en het Discord-ID hieronder blijven alleen bewaard voor de losse handmatige flow.") : (language === "en" ? "The Discord ID identifies the single payment admin who will later receive the private bot DM." : "Het Discord-ID bepaalt welke ene betaaladmin later de privé-DM van de bot ontvangt.")}</p></div></div>
      ...
      <label className="flex cursor-pointer items-center justify-between gap-4 rounded-xl bg-white/[0.025] px-4 py-3 ring-1 ring-white/[0.07]"><span className="text-sm font-semibold text-gray-200">{language === "en" ? "Enable PayPal.Me on the support page" : "PayPal.Me inschakelen op de supportpagina"}</span><input type="checkbox" checked={draftEnabled} onChange={(event) => { setDraftEnabled(event.target.checked); if (event.target.checked) setDraftCheckoutEnabled(false); }} className="h-5 w-5 accent-orange-500" /></label>
      <div className="grid gap-4 rounded-2xl bg-sky-400/[0.035] p-4 ring-1 ring-sky-300/10 sm:grid-cols-[1fr_auto] sm:items-center">
        <label className="flex cursor-pointer items-center justify-between gap-4"><span className="text-sm font-semibold text-gray-200">{language === "en" ? "Enable direct PayPal Checkout" : "Directe PayPal Checkout inschakelen"}</span><input type="checkbox" checked={draftCheckoutEnabled} onChange={(event) => { setDraftCheckoutEnabled(event.target.checked); if (event.target.checked) setDraftEnabled(false); }} className="h-5 w-5 accent-orange-500" /></label>
        <label className={label}>{language === "en" ? "Checkout environment" : "Checkoutomgeving"}<select value={draftCheckoutEnvironment} onChange={(event) => setDraftCheckoutEnvironment(event.target.value as "sandbox" | "live")} className={`${input} min-w-40`}><option value="sandbox">Sandbox</option><option value="live" disabled={!isSuperAdmin}>Live{!isSuperAdmin ? " · Super-admin" : ""}</option></select></label>
      </div>
    ...
    {localReview ? <>
      ...
      <h3 className="font-heading text-xl font-black text-white">{draftCheckoutEnabled ? (language === "en" ? "Automatic server verification" : "Automatische serververificatie") : (language === "en" ? "Verification through the private bot DM" : "Controle via de privé-DM van de bot")}</h3>
      <p className="mt-2 max-w-3xl text-sm leading-6 text-gray-400">{draftCheckoutEnabled ? (language === "en" ? "PayPal Checkout captures are verified against amount, currency and business recipient before the contribution is booked. Checkout does not create a manual payment claim or private bot DM." : "PayPal Checkout-captures worden vóór boeking gecontroleerd op bedrag, valuta en zakelijke ontvanger. Checkout maakt geen handmatige betaalclaim of privé-DM van de bot aan.") : (language === "en" ? "Open claims and their private payer details are delivered only to the configured payment admin. Confirm or reject them in that DM after checking PayPal; this page deliberately does not create a second processing path for other website admins." : "Openstaande claims en hun privé-betaalgegevens worden uitsluitend naar de ingestelde betaaladmin gestuurd. Bevestig of wijs ze in die DM af na controle in PayPal; deze pagina maakt bewust geen tweede verwerkingspad voor andere website-admins.")}</p>
    </div>}
  </section>;
};
