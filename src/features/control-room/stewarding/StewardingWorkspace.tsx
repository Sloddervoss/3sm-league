...
export function StewardingWorkspace(_: StewardingWorkspaceProps) {
  ...
  const dnfByRace = useMemo(() => (dnfQuery.data || []).reduce<Map<string, DnfResult[]>>((map, result) => { map.set(result.race_id, [...(map.get(result.race_id) || []), result]); return map; }, new Map()), [dnfQuery.data]);
  ...
  return <section aria-label="Stewarding" className="space-y-5 text-gray-100">
    ...
    {!isLoading && !queryError && tab === "dnf" && <div className="space-y-3">{[...dnfByRace.entries()].length === 0 ? <Empty icon={AlertTriangle} text="Geen DNF-resultaten om te beoordelen." /> : [...dnfByRace.entries()].map(([raceId, results]) => { const race = races.find((item) => item.id === raceId); return <article key={raceId} className="rounded-xl border border-white/[0.08] bg-white/[0.025] p-4"><h3 className="font-heading font-bold">{race?.name || "Onbekende race"}</h3><p className="mt-1 text-xs text-gray-400">{race?.track} {race ? `· ${formatDate(race.race_date)}` : ""}</p><div className="mt-4 divide-y divide-white/[0.06]">{results.map((result) => { const reviewed = (dnfPenaltiesQuery.data || []).find((penalty) => !penalty.revoked && penalty.race_id === result.race_id && penalty.user_id === result.user_id); const deduction = dnfPoints[result.id] ?? 5; return <div key={result.id} className="flex flex-wrap items-center gap-3 py-3"><AlertTriangle className="h-4 w-4 text-red-300" /><div className="min-w-[10rem] flex-1"><p className="text-sm font-bold">{nameOf(result.profiles)}</p><p className="text-xs text-gray-500">{result.points} punten · {result.laps ?? 0} ronden</p></div>{reviewed ? <><span className="rounded-full border border-white/10 px-2 py-1 text-xs text-gray-300">{reviewed.source === "abandon" ? `Abandon · −${reviewed.points_deduction}` : "Normale DNF"}</span><button type="button" disabled={mutation.isPending} onClick={() => ask({ id: "dnf-revoke", impact: "destructive", allowedRoles: [...roles], context: { resultId: result.id, penaltyId: reviewed.id, raceId: result.race_id, userId: result.user_id, source: reviewed.source } }, "DNF-beoordeling ongedaan maken?", "Punten worden hersteld voor een abandon; de penalty wordt verwijderd of ingetrokken.")} className="rounded-md border border-red-400/30 px-3 py-1.5 text-xs font-bold text-red-200 disabled:opacity-45">Ongedaan</button></> : <><button type="button" disabled={mutation.isPending} onClick={() => ask({ id: "dnf-review", impact: "write", allowedRoles: [...roles], context: { resultId: result.id, raceId: result.race_id, userId: result.user_id, outcome: "normal_dnf" } }, "Markeren als normale DNF?", "Er wordt een normale DNF-registratie zonder straf opgeslagen.")} className="rounded-md border border-emerald-400/30 px-3 py-1.5 text-xs font-bold text-emerald-100 disabled:opacity-45">Normale DNF</button><label className="flex items-center gap-1 text-xs text-gray-400">−<input type="number" min={1} max={50} value={deduction} onChange={(event) => setDnfPoints((current) => ({ ...current, [result.id]: Number(event.target.value) || 5 }))} className="w-14 rounded border border-white/10 bg-[#151820] px-2 py-1 text-center text-white"/> pt</label><button type="button" disabled={mutation.isPending} onClick={() => ask({ id: "dnf-review", impact: "write", allowedRoles: [...roles], context: { resultId: result.id, raceId: result.race_id, userId: result.user_id, outcome: "abandon", pointsDeduction: deduction } }, "Abandon-straf toepassen?", `${deduction} punten worden afgetrokken, 3SR wordt herberekend en een Cat. B penalty wordt opgeslagen.`)} className="rounded-md border border-orange-400/30 px-3 py-1.5 text-xs font-bold text-orange-100 disabled:opacity-45">Abandon</button></>}</div>; })}</div></article>; })}</div>}
