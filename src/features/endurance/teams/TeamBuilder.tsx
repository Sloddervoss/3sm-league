  ...
  return <div className="space-y-5"><Panel><SectionHeading eyebrow="Auto’s & coureurs" title="Team Builder" description="Maak teams en verdeel de beschikbare coureurs. Sleep daarna handmatig bij." />
    ...
    <div className="grid gap-4 xl:grid-cols-[0.8fr_repeat(2,1fr)]">
      ...
      {teams.map((team) => {
        ...
        return <div key={team.id} onDragOver={(e) => manager && e.preventDefault()} onDrop={(e) => drop(e, team.id)} className="rounded-2xl bg-gradient-to-br from-orange-500/[0.08] to-black/20 p-4 ring-1 ring-orange-500/15"><div className="mb-4 flex items-start justify-between gap-3"><div><div className="flex items-center gap-2"><Car className="h-4 w-4 text-orange-400" /><h3 className="font-heading text-lg font-black text-white">{team.name} #{team.car_number}</h3></div><p className="mt-1 text-xs text-gray-500">{car?.classId} · {car?.name}</p></div><StatusPill tone="orange">{teamMembers.length}/{car?.maxDrivers ?? event.maxDriversPerCar} coureurs</StatusPill></div><div className="space-y-2">{teamMembers.map((m) => <DriverCard key={m.id} id={m.user_id} name={personName(m.user_id)} role={m.role} draggable={manager} />)}</div></div>;
      })}
    </div>
  </Panel></div>;
};
