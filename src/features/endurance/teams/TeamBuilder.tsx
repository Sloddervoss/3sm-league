...
export const TeamBuilder = ({ event }: { event: EnduranceEvent }) => {
  ...
  const canEditTeams = teams.some((team) => canManageTeam(state, team, activePersona));
  ...
  const create = (formEvent: React.FormEvent) => { formEvent.preventDefault(); if (!name || !number || !event.selectedCarId) return; dispatch({ type: "create_team", team: { id: makeId("team"), eventId: event.id, name, carId: event.selectedCarId, carNumber: number, managerId: activePersona.id, livery: "" } }); setName(""); setNumber(""); };
  ...
  const unassign = (userId: string) => {
    const membership = state.teamMembers.find((member) => member.userId === userId && teams.some((team) => team.id === member.teamId));
    const team = membership && teams.find((candidate) => candidate.id === membership.teamId);
    if (!membership || !team || !canManageTeam(state, team, activePersona)) return;
    dispatch({ type: "remove_team_member", teamId: team.id, userId });
  };
  ...
  return <div className="space-y-5"><Panel><SectionHeading eyebrow="Auto’s & coureurs" title="Team Builder" description="Maak pacegroepen of verdeel de beschikbare coureurs zo gelijk mogelijk. Sleep daarna handmatig bij." action={manager && <div className="flex flex-wrap gap-2"><SecondaryButton onClick={() => auto("balanced")} disabled={!teams.length}><Shuffle className="h-4 w-4" /> Gelijke teams</SecondaryButton><PrimaryButton onClick={() => auto("pace_groups")} disabled={!teams.length}><Users className="h-4 w-4" /> Pacegroepen</PrimaryButton></div>} />
    ...
    <div className="grid gap-4 xl:grid-cols-[0.8fr_repeat(2,1fr)]">
      <div
        onDragOver={(dragEvent) => { if (canEditTeams) { dragEvent.preventDefault(); setUnassignedDragOver(true); } }}
        onDragLeave={(dragEvent) => { if (!dragEvent.currentTarget.contains(dragEvent.relatedTarget as Node | null)) setUnassignedDragOver(false); }}
        onDrop={dropUnassigned}
        className={`rounded-2xl p-4 ring-1 transition ${unassignedDragOver ? "bg-orange-500/[0.12] ring-orange-400/40" : "bg-black/20 ring-white/5"}`}
      >
        <h3 className="font-heading font-black text-white">Nog in te delen</h3>
        {canEditTeams && <p className="mb-3 mt-1 text-xs text-gray-500">Sleep een coureur hierheen om die uit een team te halen.</p>}
        <div className="space-y-2">{unassigned.map((id) => <DriverCard key={id} id={id} name={person(id)?.name ?? id} pace={pace(id)?.averageLapSeconds} draggable={canEditTeams} />)}{!unassigned.length && <p className="rounded-xl border border-dashed border-white/10 p-4 text-center text-xs text-gray-500">Iedereen is ingedeeld. Sleep een coureur hierheen om de indeling ongedaan te maken.</p>}</div>
      </div>
      {teams.map((team) => { const members = state.teamMembers.filter((member) => member.teamId === team.id); const car = getEnduranceCar(team.carId); const manageable = canManageTeam(state, team, activePersona); const averages = members.map((member) => pace(member.userId)?.averageLapSeconds).filter((value): value is number => Boolean(value)); const average = averages.length ? averages.reduce((sum, value) => sum + value, 0) / averages.length : null; return <div key={team.id} onDragOver={(e) => manageable && e.preventDefault()} onDrop={(e) => drop(e, team.id)} className="rounded-2xl bg-gradient-to-br from-orange-500/[0.08] to-black/20 p-4 ring-1 ring-orange-500/15"><div className="mb-4 flex items-start justify-between gap-3"><div><div className="flex items-center gap-2"><Car className="h-4 w-4 text-orange-400" /><h3 className="font-heading text-lg font-black text-white">{team.name} #{team.carNumber}</h3></div><p className="mt-1 text-xs text-gray-500">{car?.classId} · {car?.name}</p></div><StatusPill tone={teamCoveragePercent(state, event, team.id) >= 100 ? "green" : "orange"}>{teamCoveragePercent(state, event, team.id)}% dekking</StatusPill></div><div className="space-y-2">{members.map((member) => <DriverCard key={member.id} id={member.userId} name={person(member.userId)?.name ?? member.userId} pace={pace(member.userId)?.averageLapSeconds} role={member.role} draggable={manageable} />)}</div><div className="mt-4 flex justify-between text-xs text-gray-500"><span>{members.length}/{car?.maxDrivers ?? event.maxDriversPerCar} coureurs</span><span>{average ? `Gem. ${formatLapTime(average)}` : "Geen pace"}</span></div></div>; })}
    </div>
  </Panel></div>;
};
