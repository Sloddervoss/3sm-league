...
import { delayedStintUpdates, stintRowToUpsert } from "./raceControlUpdates";
...
export const RaceControlPanel = ({ event }: { event: EnduranceEvent }) => {
  ...
  const { upsert, applyBatch } = useEnduranceStintMutations(event.id);
  ...
  const editable = Boolean(team && (isSuperAdmin || isEnduranceManager));
  ...
  const delay = async (minutes: number) => {
    const updates = delayedStintUpdates(stintRows.filter((row) => row.team_id === teamId), nowIso, minutes);
    if (!updates.length) { setFeedback("Geen actieve of toekomstige stints om te verschuiven."); return; }
    try {
      await applyBatch.mutateAsync({ teamId, stints: updates });
      setFeedback(`${updates.length} actieve/toekomstige stint(s) ${minutes} minuten gecorrigeerd. Originele planning is bewaard.`);
    } catch (error) {
  ...
  const complete = async () => {
    ...
    try {
      await upsert.mutateAsync(stintRowToUpsert(currentRow, { status: "completed", actual_end_at: nowIso }));
      setFeedback("Huidige stint voltooid; de actuele eindtijd is opgeslagen.");
    } catch (error) {
      setFeedback(`Stint beëindigen mislukt: ${(error as Error)?.message ?? String(error)}`);
    }
  };
  const replace = async () => {
    if (!currentRow || !replacement) return;
    try {
      await upsert.mutateAsync(stintRowToUpsert(currentRow, {
        driver_id: replacement,
        status: "replaced",
        notes: `${currentRow.notes ?? ""}${currentRow.notes ? " · " : ""}vervangen door Race Control`,
      }));
      setFeedback(`Coureur vervangen door ${driver(replacement)}.`);
    } catch (error) {
      setFeedback(`Coureur vervangen mislukt: ${(error as Error)?.message ?? String(error)}`);
    }
  };
  ...
  {editable && <Panel><SectionHeading title="Handmatige correcties" description="Pas toekomstige stints aan of vervang de huidige coureur. Iedere wijziging komt in het auditlog." /><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3"><Field label="Aangepaste vertraging"><div className="flex gap-2"><input type="number" min={-60} max={180} className={inputClass} value={customDelay} onChange={(e) => setCustomDelay(Number(e.target.value))} /><SecondaryButton onClick={() => void delay(customDelay)} disabled={upsert.isPending || applyBatch.isPending}><Clock3 className="h-4 w-4" /> Toepassen</SecondaryButton></div></Field><Field label="Extra repairtijd"><div className="flex gap-2"><input type="number" min={0} max={180} className={inputClass} value={customDelay} onChange={(e) => setCustomDelay(Number(e.target.value))} /><SecondaryButton onClick={() => void delay(customDelay)} disabled={upsert.isPending || applyBatch.isPending}><Wrench className="h-4 w-4" /> Toevoegen</SecondaryButton></div></Field><Field label="Coureur vervangen"><div className="flex gap-2"><select className={inputClass} value={replacement} onChange={(e) => setReplacement(e.target.value)}><option value="">Selecteer</option>{teamDrivers.map((m) => <option key={m.user_id} value={m.user_id}>{displayName(m.user_id)}</option>)}</select><SecondaryButton onClick={() => void replace()} disabled={!replacement || upsert.isPending || applyBatch.isPending}>Vervangen</SecondaryButton></div></Field></div>{feedback && <p role="status" className="mt-4 flex items-center gap-2 text-sm text-orange-200"><AlertTriangle className="h-4 w-4" />{feedback}</p>}</Panel>}
  </div>;
};
