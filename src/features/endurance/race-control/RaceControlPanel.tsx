...
import { canManageEvent, canManageTeam, formatAmsterdam } from "../core/selectors";
...
export const RaceControlPanel = ({ event }: { event: EnduranceEvent }) => {
  ...
  const teams = state.teams.filter((team) => team.eventId === event.id && (canManageEvent(event, activePersona) || team.managerId === activePersona.id || state.teamMembers.some((member) => member.teamId === team.id && member.userId === activePersona.id)));
  ...
  const stints = useMemo(() => state.stints.filter((stint) => stint.eventId === event.id && stint.teamId === teamId).sort((a, b) => a.actualStartAt.localeCompare(b.actualStartAt)), [state.stints, event.id, teamId]);
  ...
  const driver = (id?: string) => state.personas.find((persona) => persona.id === id)?.name ?? "Nog niet gepland";
  const teamDrivers = state.teamMembers.filter((member) => member.teamId === teamId).map((member) => state.personas.find((persona) => persona.id === member.userId)).filter(Boolean);
  ...
  const delay = (minutes: number) => { dispatch({ type: "adjust_future_stints", eventId: event.id, teamId, fromAt: current?.actualStartAt ?? nowIso, deltaMinutes: minutes }); setFeedback(`Alle toekomstige stints ${minutes} minuten verschoven. Originele planning blijft bewaard.`); };
  const complete = () => { if (!current) return; dispatch({ type: "complete_stint", stintId: current.id, completedAt: nowIso }); setFeedback("Huidige stint voltooid en actuele eindtijd opgeslagen."); };
  const replace = () => { if (!current || !replacement) return; dispatch({ type: "upsert_stint", stint: { ...current, driverId: replacement, status: "replaced", notes: `${current.notes} · vervangen door Race Control` } }); setFeedback(`Coureur vervangen door ${driver(replacement)}.`); };

  if (!teams.length) return <Panel><p className="text-sm text-gray-400">Race Control wordt beschikbaar zodra je aan een auto bent gekoppeld.</p></Panel>;
  return <div className="space-y-5"><Panel className="overflow-hidden"><SectionHeading eyebrow="Live handmatige besturing" title="Race Control" description="Geen telemetrie nodig: corrigeer de actuele planning terwijl de oorspronkelijke planning intact blijft." action={<StatusPill tone={event.status === "live" ? "red" : "orange"}>{event.status === "live" ? "Race live" : "Oefenmodus"}</StatusPill>} />
    <div className="mb-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Field label="Team"><select className={inputClass} value={teamId} onChange={(e) => setTeamId(e.target.value)}>{teams.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.name} #{candidate.carNumber}</option>)}</select></Field><Field label="Actuele racetijd (Nederland)"><input type="datetime-local" className={inputClass} value={currentAt} onChange={(e) => setCurrentAt(e.target.value)} /></Field><div className="rounded-xl bg-black/20 p-3"><span className="text-xs text-gray-500">Verschil met origineel</span><strong className={`mt-1 block text-2xl ${deltaMinutes ? "text-orange-300" : "text-emerald-300"}`}>{deltaMinutes > 0 ? "+" : ""}{deltaMinutes} min</strong></div><div className="rounded-xl bg-black/20 p-3"><span className="text-xs text-gray-500">Resterende racetijd</span><strong className="mt-1 block text-2xl text-white">{Math.max(0, Math.ceil((new Date(event.endAt).getTime() - new Date(nowIso).getTime()) / 3_600_000))} uur</strong></div></div>
