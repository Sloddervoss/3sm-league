 ...
 */
export const RaceControlPanel = ({ event, selectedTeamId, embedded = false }: { event: EnduranceEvent; selectedTeamId?: string; embedded?: boolean }) => {
  ...
  const [localTeamId, setTeamId] = useState(teams[0]?.id ?? "");
  ...
  const nowIso = liveTime ? now : (() => { try { return zonedInputToUtc(currentAt); } catch { return event.startAt; } })();
  ...
  const activeIndex = stints.findIndex((stint) => isCurrentStint(stint.status, stint.actualStartAt, stint.actualEndAt, nowIso));
  ...
  if (!teams.length) return <div className="space-y-5"><Panel><p className="text-sm text-gray-400">Race Control wordt beschikbaar zodra je aan een auto bent gekoppeld.</p></Panel>{!embedded && <SimHubTelemetryPanel eventId={event.id} teamId={teamId} plannedDriverId={undefined} />}</div>;
  return <div className="space-y-5"><Panel className="overflow-hidden"><SectionHeading eyebrow="Live handmatige besturing" title="Race Control" description="Volg de huidige stint en de volgende coureurs. Managers kunnen de planning hier bijsturen, ook zonder telemetrie." action={<StatusPill tone={event.status === "live" ? "red" : "orange"}>{event.status === "live" ? "Race live" : "Oefenmodus"}</StatusPill>} />
    <div className="mb-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Field label="Team"><select disabled={selectedTeamId !== undefined} className={inputClass} value={teamId} onChange={(e) => setTeamId(e.target.value)}>{teams.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.name} #{candidate.car_number}</option>)}</select></Field><div><p className="mb-2 text-xs font-bold text-gray-400">Actuele racetijd (Nederland)</p><label className="mb-2 flex items-center gap-2 text-xs text-gray-300"><input type="checkbox" checked={liveTime} onChange={(e) => { setLiveTime(e.target.checked); setCurrentAt(utcToZonedInput(now)); }} /> Live tijd volgen</label><input aria-label="Actuele racetijd (Nederland)" type="datetime-local" disabled={liveTime} className={inputClass} value={liveTime ? utcToZonedInput(now) : currentAt} onChange={(e) => setCurrentAt(e.target.value)} /></div><div className="rounded-xl bg-black/20 p-3"><span className="text-xs text-gray-500">Verschil met origineel</span><strong className={`mt-1 block text-2xl ${deltaMinutes ? "text-orange-300" : "text-emerald-300"}`}>{deltaMinutes > 0 ? "+" : ""}{deltaMinutes} min</strong></div><div className="rounded-xl bg-black/20 p-3"><span className="text-xs text-gray-500">Resterende racetijd</span><strong className="mt-1 block text-2xl text-white">{Math.max(0, Math.ceil((new Date(event.endAt).getTime() - new Date(nowIso).getTime()) / 3_600_000))} uur</strong></div></div>
  ...
  {!embedded && <SimHubTelemetryPanel eventId={event.id} teamId={teamId} plannedDriverId={current?.driverId} />}
  {editable && <Panel><SectionHeading title="Handmatige correcties" description="Pas de planning aan bij vertraging, reparatie of een rijderswissel. Alle wijzigingen worden bewaard." /><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3"><Field label="Aangepaste vertraging (minuten)"><div className="flex gap-2"><input type="number" min={-60} max={180} className={inputClass} value={customDelay} onChange={(e) => setCustomDelay(Number(e.target.value))} /><SecondaryButton onClick={() => void delay(customDelay)} disabled={raceControlApply.isPending}><Clock3 className="h-4 w-4" /> Toepassen</SecondaryButton></div></Field><Field label="Extra repairtijd (seconden)"><div className="flex gap-2"><input type="number" min={1} max={1800} className={inputClass} value={repairSeconds} onChange={(e) => setRepairSeconds(Number(e.target.value))} /><SecondaryButton onClick={() => void repair(repairSeconds)} disabled={!currentRow || raceControlApply.isPending}><Wrench className="h-4 w-4" /> Toevoegen</SecondaryButton></div></Field><Field label="Coureur vervangen"><div className="flex gap-2"><select className={inputClass} value={replacement} onChange={(e) => setReplacement(e.target.value)}><option value="">Selecteer</option>{teamDrivers.map((m) => <option key={m.user_id} value={m.user_id}>{displayName(m.user_id)}</option>)}</select><SecondaryButton onClick={() => void replace()} disabled={!replacement || raceControlApply.isPending}>Vervangen</SecondaryButton></div></Field></div>
    ...
    {auditRows.length > 0 && <div className="mt-4"><h4 className="text-xs font-black uppercase tracking-wider text-gray-500">Recente wijzigingen</h4><ul className="mt-2 space-y-1.5">{auditRows.slice(0, 5).map((entry) => <li key={entry.id} className="flex items-center justify-between rounded-lg bg-white/[0.03] px-3 py-1.5"><span className="text-xs text-gray-300">{auditLabel(entry)} → {entry.after_status}</span><span className="text-[11px] text-gray-500">{formatAmsterdam(entry.created_at)}</span></li>)}</ul></div>}
  </Panel>}
  </div>;
};