...
const ActivationPanel = ({ event, slot, onClose }: { event: IRacingCatalogEvent; slot: IRacingCatalogSlot; onClose: () => void }) => {
  ...
  return <Panel className="mt-4 bg-black/45 ring-orange-400/25">
    ...
    <form onSubmit={submit} className="grid gap-4 md:grid-cols-2">
      ...
      <Field label="Toegestaan maximum per auto" hint="Gewenste teamgroottes kies je later per team."><input required type="number" min={1} max={30} className={inputClass} value={maxDrivers} onChange={(event) => setMaxDrivers(Math.max(1, Math.min(30, Number(event.target.value) || 1)))} /></Field>
