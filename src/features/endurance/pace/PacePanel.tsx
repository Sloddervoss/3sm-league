...
import type { EnduranceEvent, PaceEntry } from "../core/types";
...
export const PacePanel = ({ event }: { event: EnduranceEvent }) => {
  ...
  const addManual = async (formEvent: React.FormEvent) => {
    ...
    try {
      ...
      if (!Number.isInteger(laps) || laps < 1) throw new Error("Vul het aantal geldige ronden in.");
      await persist({
        event_id: event.id,
        user_id: actorId,
        ...
        best_five_average_seconds: null,
