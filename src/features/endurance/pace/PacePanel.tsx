...
export const PacePanel = ({ event }: { event: EnduranceEvent }) => {
  ...
  return <div className="space-y-5"><Panel><SectionHeading eyebrow="Meerdere ronden" title="Pace & betrouwbaarheid" description="Rangschikking gebruikt gemiddelde, consistentie, incidenten en hoeveelheid data—nooit alleen de snelste ronde." />
    {!isLoading && <div className="space-y-3 sm:hidden">{sorted.map((entry) => <article key={entry.id} className="rounded-xl bg-black/20 p-4 ring-1 ring-white/5"><div className="flex items-center justify-between gap-2"><strong className="text-sm text-white">{displayName(entry.user_id)}</strong>{entry.source === "practice" && <StatusPill tone="green">Practice</StatusPill>}</div><p className="mt-1 text-xs text-gray-500">{entry.car}</p><dl className="mt-4 grid grid-cols-2 gap-3 text-xs"><div><dt className="text-gray-500">Gemiddelde</dt><dd className="mt-1 font-bold text-orange-300">{formatLapTime(entry.average_lap_seconds ?? 0)}</dd></div><div><dt className="text-gray-500">Beste 5</dt><dd className="mt-1 font-bold text-white">{formatLapTime(entry.best_five_average_seconds ?? 0)}</dd></div><div><dt className="text-gray-500">Ronden</dt><dd className="mt-1 text-gray-200">{entry.valid_laps}</dd></div><div><dt className="text-gray-500">Afwijking</dt><dd className="mt-1 text-gray-200">±{(entry.consistency_seconds ?? 0).toFixed(2)}s</dd></div></dl></article>)}</div>}
