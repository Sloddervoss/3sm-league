...
export const PacePanel = ({ event }: { event: EnduranceEvent }) => {
  ...
  const addManual = (formEvent: React.FormEvent) => { formEvent.preventDefault(); if (!selectedCar) { setFeedback("Bevestig eerst de definitieve auto in het overzicht."); return; } try { const averageLapSeconds = parseLapSeconds(average); const bestLapSeconds = parseLapSeconds(best); dispatch({ type: "add_pace_entry", entry: { id: makeId("pace"), eventId: event.id, userId: manager ? userId : activePersona.id, circuit: event.circuit, configuration: event.configuration, car: selectedCar.name, conditions: "dry", averageLapSeconds, medianLapSeconds: averageLapSeconds, bestLapSeconds, bestFiveAverageSeconds: (averageLapSeconds + bestLapSeconds) / 2, consistencySeconds: consistency, validLaps: laps, incidents, averageStintMinutes: 0, recordedAt: new Date().toISOString(), source: "manual", notes: "" } }); setFeedback("Pacegegevens opgeslagen."); } catch (error) { setFeedback(error instanceof Error ? error.message : "Pace kon niet worden opgeslagen."); } };

  const upload = async (file?: File) => { if (!file) return; if (!selectedCar) { setFeedback("Bevestig eerst de definitieve auto in het overzicht."); return; } try { const parsed = parsePaceCsv(await file.text()); dispatch({ type: "add_pace_entry", entry: { id: makeId("pace"), eventId: event.id, userId: manager ? userId : activePersona.id, circuit: event.circuit, configuration: event.configuration, car: selectedCar.name, conditions: "dry", ...parsed, recordedAt: new Date().toISOString(), source: "csv", notes: file.name } }); setFeedback(`${file.name} verwerkt zonder gedeeltelijke import.`); } catch (error) { setFeedback(error instanceof Error ? error.message : "CSV kon niet worden verwerkt."); } };
