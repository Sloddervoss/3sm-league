...
import { RoadAtlantaProjection } from './RoadAtlantaProjection';
...
export function LiveTrackPanel({ v3, live, fallbackTrack }: { v3: V3Normalized | null; live: boolean; fallbackTrack: string }) {
  ...
  const roadAtlantaFull = /road atlanta/i.test(track) && /full course/i.test(`${track} ${config}`);
  ...
  return <section data-pitwall-slot="trackmap" className="rounded-xl border border-white/10 bg-black/40 p-4">
    ...
    {roadAtlantaFull ? <RoadAtlantaProjection v3={v3} live={live} /> : <TrackMap track={track} className="h-52 w-full object-contain" />}
    ...
    <p className="text-[10px] text-gray-500">{cars.length ? `${cars.length} auto’s met positie. ` : 'Wacht op autoposities. '}{!roadAtlantaFull && 'Plaatsing op de circuitvorm is nog niet gekalibreerd voor deze layout.'}</p>
  </section>;
}
