...
import type { IRacingCatalogCar, IRacingCatalogEvent, IRacingCatalogSlot, IRacingInterestSummaryRow } from "../calendar/iracingCatalogPresentation";
...
export const iracingInterestSummaryQueryKey = ["endurance", "iracing-interest-summary"] as const;
...
/** Laadt de pre-activatie-interesse per event (aantal geïnteresseerde leden + jijzelf). */
export async function listIRacingInterestSummary(): Promise<IRacingInterestSummaryRow[]> {
  assertEnduranceTable("endurance_iracing_events");
...
export function useIRacingInterestSummary() {
  return useQuery({ queryKey: iracingInterestSummaryQueryKey, queryFn: listIRacingInterestSummary });
}

/** Zet de pre-activatie-interesse van de ingelogde gebruiker voor een event aan/uit. */
export async function setIRacingInterest(catalogEventId: string, interested: boolean): Promise<void> {
  assertEnduranceTable("endurance_iracing_events");
...
export function useSetIRacingInterest() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ catalogEventId, interested }: { catalogEventId: string; interested: boolean }) =>
      setIRacingInterest(catalogEventId, interested),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: iracingInterestSummaryQueryKey }),
  });
}
