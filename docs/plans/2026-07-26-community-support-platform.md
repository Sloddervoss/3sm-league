...
- After JSON is read (or a manual correction is prepared), the main result-import screen immediately shows editable hosted hours, optional 25% discount and a live USD/EUR preview. The final confirmation is summary-only. After a successful result write, it create-only initializes a missing race-cost record for the same `race_id`; an explicitly changed existing booking is updated only after that successful write while retaining its original exchange-rate snapshot. An unchanged re-import performs no financial write.
...
- The Control Room supports explicit race-cost corrections both during result import and in Race Costs, plus one bulk hours/discount update for every recorded race in a selected season; standalone races remain individually editable. Every explicit correction preserves the stored exchange rate.
