-- Telemetry V3 Phase F rollback.
-- Removes ONLY Phase-F-owned objects.
-- Restores Phase E simhub_persist_v3 to exact original definition.
\set ON_ERROR_STOP on
BEGIN;

-- Remove Phase F objects
DROP FUNCTION IF EXISTS public.simhub_update_strategy_v3(uuid,uuid,text,bigint,uuid,uuid,integer,numeric,boolean,numeric,integer,boolean,integer,boolean);
DROP TABLE IF EXISTS public.endurance_strategy_lap_samples;
DROP TABLE IF EXISTS public.endurance_strategy_latest;
DROP SEQUENCE IF EXISTS public.endurance_fuel_stint_seq;

-- Restore exact Phase E simhub_persist_v3 (original definition from Phase E migration)
