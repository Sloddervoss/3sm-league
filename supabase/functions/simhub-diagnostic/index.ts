...
const assertString = (value: unknown, field: string, maxLen = 120): string => {
  if (typeof value !== "string" || !value.trim() || value.length > maxLen) throw new Error("invalid_payload");
  return value.trim();
};
...
const assertBoolean = (value: unknown, field: string): boolean => {
  if (typeof value !== "boolean") throw new Error("invalid_payload");
  return value;
};
...
const assertNumber = (
  ...
  if (options.integer && !Number.isInteger(value)) throw new Error("invalid_payload");
  return value;
};

const assertTimestampOrNull = (value: unknown): string | null => {
  if (value === null) return null;
  if (typeof value !== "string" || !value.trim() || value.length > 40) throw new Error("invalid_payload");
  if (!Number.isFinite(Date.parse(value.trim()))) throw new Error("invalid_payload");
  return value.trim();
};

const assertDiagnosticCode = (value: unknown): string => {
  if (typeof value !== "string" || !value.trim()) throw new Error("invalid_payload");
  if (!diagnosticCodes.has(value.trim())) throw new Error("invalid_payload");
  return value.trim();
};
...
const assertDetail = (value: unknown): string | null => {
  ...
  if (value.length > 200) throw new Error("invalid_payload");
...
const assertExceptionType = (value: unknown): string | null => {
  if (value === null) return null;
  if (typeof value !== "string" || !value.trim() || value.length > 200) throw new Error("invalid_payload");
  // Only valid exception type names: dot-separated identifiers
  if (!/^[A-Za-z0-9_.+`]+$/.test(value.trim())) throw new Error("invalid_payload");
  return value.trim();
};
...
const validateHeartbeat = (body: Record<string, unknown>): void => {
  ...
  if (body.type !== "heartbeat") throw new Error("invalid_payload");
  ...
  assertString(body.clientReportedAtUtc, "clientReportedAtUtc", 40);
};

const validateEvent = (body: Record<string, unknown>): void => {
  exactKeys(body, eventKeys);
  if (body.type !== "event") throw new Error("invalid_payload");
  assertString(body.deviceId, "deviceId", 40);
  assertDiagnosticCode(body.code);
  assertString(body.atUtc, "atUtc", 40);
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (known[message]) {
      return jsonResponse(request, { error: message }, known[message]);
    }
