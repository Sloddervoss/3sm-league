-- Endurance Fase 2B: ingest-routing en identiteitskolommen.
-- Additief: slaat de v2-identiteit op, past voor endurance-gebonden devices de
-- registered-gate toe en leidt een voltooide ronde door naar een actieve
-- practice-sessie. Ongebonden/legacy devices blijven exact het huidige gedrag
-- volgen (latest-only, geen gate).
BEGIN;

-- Identiteitskolommen op de latest-snapshot zodat Race Control per team live
-- coureur/auto/circuit kan tonen zonder de plugin-identiteit te vertrouwen als
-- autoriteit (event/team blijven server-side afgeleid).
ALTER TABLE public.simhub_telemetry_latest
  ADD COLUMN IF NOT EXISTS endurance_event_id UUID,
  ADD COLUMN IF NOT EXISTS endurance_team_id UUID,
  ADD COLUMN IF NOT EXISTS driver_id TEXT,
  ADD COLUMN IF NOT EXISTS current_driver_id TEXT,
  ADD COLUMN IF NOT EXISTS current_driver_name TEXT,
  ADD COLUMN IF NOT EXISTS car_id TEXT,
  ADD COLUMN IF NOT EXISTS car_name TEXT,
  ADD COLUMN IF NOT EXISTS track_name TEXT,
  ADD COLUMN IF NOT EXISTS track_config TEXT;
