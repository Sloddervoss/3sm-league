-- Telemetry V3 Phase E: additive persistence against recovered production schema.
-- Shared base tables are never created or dropped here.
\set ON_ERROR_STOP on
BEGIN;

ALTER TABLE public.endurance_telemetry_events
  ADD COLUMN IF NOT EXISTS race_run_id uuid REFERENCES public.endurance_race_runs(id) ON DELETE SET NULL;
ALTER TABLE public.simhub_telemetry_latest
  ADD COLUMN IF NOT EXISTS race_run_id uuid REFERENCES public.endurance_race_runs(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS v3_normalized JSONB;
ALTER TABLE public.simhub_telemetry_latest
  ADD CONSTRAINT simhub_telemetry_latest_v3_normalized_object_check
  CHECK (v3_normalized IS NULL OR jsonb_typeof(v3_normalized) = 'object') NOT VALID;
ALTER TABLE public.simhub_telemetry_latest VALIDATE CONSTRAINT simhub_telemetry_latest_v3_normalized_object_check;

-- Exact recovered constraint, with only the authorized new V3 semantic.
...
'fuel_added'::text,'driver_change'::text,'stint_start'::text,'stint_end'::text,'flag_change'::text,
'car_state_change'::text,'incident_count_changed'::text]));

CREATE INDEX IF NOT EXISTS endurance_telemetry_events_race_run_idx ON public.endurance_telemetry_events (race_run_id, received_at DESC) WHERE race_run_id IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS endurance_telemetry_events_v3_lap_dedupe_idx ON public.endurance_telemetry_events (race_run_id, completed_laps) WHERE race_run_id IS NOT NULL AND event_type = 'lap_completed' AND completed_laps IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS endurance_telemetry_events_v3_incident_dedupe_idx ON public.endurance_telemetry_events (race_run_id, incidents) WHERE race_run_id IS NOT NULL AND event_type = 'incident_count_changed' AND incidents IS NOT NULL;
CREATE INDEX IF NOT EXISTS simhub_telemetry_latest_race_run_idx ON public.simhub_telemetry_latest (race_run_id, received_at DESC) WHERE race_run_id IS NOT NULL;

CREATE TABLE public.endurance_source_segments (
 ...
 race_run_id uuid REFERENCES public.endurance_race_runs(id) ON DELETE SET NULL,
 last_sequence bigint NOT NULL CHECK(last_sequence >= 0),
 previous_completed_laps integer,
 previous_in_pit_lane boolean,
 previous_incidents integer,
 previous_flags text[],
 created_at timestamptz NOT NULL DEFAULT clock_timestamp(),
 updated_at timestamptz NOT NULL DEFAULT clock_timestamp(),
 PRIMARY KEY(device_id,session_id,event_id,team_id)
);
ALTER TABLE public.endurance_source_segments ENABLE ROW LEVEL SECURITY;
CREATE POLICY endurance_source_segments_service_role_all ON public.endurance_source_segments FOR ALL TO service_role USING (true) WITH CHECK (true);
REVOKE ALL ON public.endurance_source_segments FROM PUBLIC, anon, authenticated;
GRANT ALL ON public.endurance_source_segments TO service_role;
...
BEGIN
 ...
 -- Reuse the canonical V2 transport-session monotone guard. A retry is
 -- rejected before any segment/event/latest mutation.
