-- Harden 3SR recalculation RPCs.
--
-- These functions remain callable by the existing authenticated admin/steward UI
-- and service-role automation, but the SECURITY DEFINER body now performs an
-- explicit privileged-role check before any recalculation writes occur. This
-- prevents a normal authenticated user from using the broad EXECUTE grant as a
-- write-capable RPC path.
...
BEGIN
  ...
  IF NOT (
    ...
    RAISE EXCEPTION 'Not allowed to recalculate 3SR'
      USING ERRCODE = '42501';
  END IF;
...
-- Aanroepen via RPC is alleen mogelijk voor admin/super_admin/moderator of service_role.
-- -----------------------------------------------------------------------------

REVOKE ALL ON FUNCTION public.recalculate_3sr_for_race(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.recalculate_3sr_all() FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.recalculate_3sr_for_race(uuid)
  TO authenticated, service_role;

GRANT EXECUTE ON FUNCTION public.recalculate_3sr_all()
  TO authenticated, service_role;
